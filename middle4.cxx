...
int main()
{
	...
	double val[11][1000]={0.0,};
	double mini[1000]={0.0,};
	for(int j =0;j<11;j++){
		double *instant_val=cal(width[j],rebin);
	...
	for(int i =0;i<1000;i++){
	    double y_vals[11] = {val[0 ][i]-mini[i],
			                 val[1 ][i]-mini[i],
							 val[2 ][i]-mini[i],
							 val[3 ][i]-mini[i],
							 val[4 ][i]-mini[i],
	...
	mini[1000]={0.0,};
	for(int j =0;j<11;j++){
		double *instant_val=cal_16(width[j],rebin);
		for(int k =0;k<1000;k++) {
			val[j][k] =instant_val[k];
			if(mini[k] > val[j][k]) mini[k] = val[j][k];
		}
