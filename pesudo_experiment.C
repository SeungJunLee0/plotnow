...
void fcn(int &npar, double *gin, double &f, double *par, int iflag) {
    ...
    for (int i = 0; i <= hist_data->GetNbinsX(); ++i) {
        double dataContent = hist_data->GetBinContent(i+1);
        double mcContent =   hist_mc_1->GetBinContent(i+1);
        if (par[0]*dataContent > 0.0) {
...
double cal(TString width, int count1) {
	...
	double value[5],sstep,bbmin,bbmax;
	TString ds;
	gMinuit->mnpout(0, ds, value[0], sstep,bbmin , bbmax,ierflg);
